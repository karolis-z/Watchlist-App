...
@Dao
abstract class SearchAllCacheDao :
    BaseCacheDao<TitleItemCacheSearchAll, GenreForCacheItemSearchAll, RemoteKeySearchAll> {
    ...
    @Query("SELECT * FROM 'title_item_cache_search_all' ORDER BY page, id")
    abstract fun getCachedTitles(): PagingSource<Int, TitleItemCacheSearchAllFull>

    @Query("DELETE FROM 'title_item_cache_search_all'")
    abstract suspend fun clearAllCachedTitles()

    @Query("SELECT * FROM 'remote_key_search_all' WHERE cachedTitleId = :id")
    abstract suspend fun getRemoteKeyById(id: Long): RemoteKeySearchAll?

    @Query("DELETE FROM 'remote_key_search_all'")
    abstract suspend fun clearRemoteKeys()

    @Query("SELECT createdOn FROM 'remote_key_search_all' ORDER BY createdOn DESC LIMIT 1")
