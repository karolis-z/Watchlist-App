...
@Dao
abstract class UpcomingMoviesCacheDao :
    BaseCacheDao<TitleItemCacheUpcomingMovie, GenreForCacheItemUpcomingMovie, RemoteKeyUpcomingMovie> {
    ...
    @Query("SELECT * FROM 'title_item_cache_upcoming_movie' ORDER BY page, id")
    abstract fun getCachedTitles(): PagingSource<Int, TitleItemCacheUpcomingMovieFull>

    @Query("DELETE FROM 'title_item_cache_upcoming_movie'")
    abstract suspend fun clearAllCachedTitles()

    @Query("SELECT * FROM 'remote_key_upcoming_movie' WHERE cachedTitleId = :id")
    abstract suspend fun getRemoteKeyById(id: Long): RemoteKeyUpcomingMovie?

    @Query("DELETE FROM 'remote_key_upcoming_movie'")
    abstract suspend fun clearRemoteKeys()

    @Query("SELECT createdOn FROM 'remote_key_upcoming_movie' ORDER BY createdOn DESC LIMIT 1")
