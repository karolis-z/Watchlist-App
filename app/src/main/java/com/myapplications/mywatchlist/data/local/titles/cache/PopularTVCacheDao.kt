...
@Dao
abstract class PopularTVCacheDao :
    ...
    @Query("SELECT * FROM 'title_item_cache_popular_tv' ORDER BY page, id")
    abstract fun getCachedTitles(): PagingSource<Int, TitleItemCachePopularTVFull>

    @Query("DELETE FROM 'title_item_cache_popular_tv'")
    abstract suspend fun clearAllCachedTitles()

    @Query("SELECT * FROM 'remote_key_popular_tv' WHERE cachedTitleId = :id")
    abstract suspend fun getRemoteKeyById(id: Long): RemoteKeyPopularTV?

    @Query("DELETE FROM 'remote_key_popular_tv'")
    abstract suspend fun clearRemoteKeys()

    @Query("SELECT createdOn FROM 'remote_key_popular_tv' ORDER BY createdOn DESC LIMIT 1")
