...
@Dao
abstract class TopRatedTVCacheDao :
    BaseCacheDao<TitleItemCacheTopRatedTV, GenreForCacheItemTopRatedTV, RemoteKeyTopRatedTV> {
    ...
    @Query("SELECT * FROM 'title_item_cache_toprated_tv' ORDER BY page, id")
    abstract fun getCachedTitles(): PagingSource<Int, TitleItemCacheTopRatedTVFull>

    @Query("DELETE FROM 'title_item_cache_toprated_tv'")
    abstract suspend fun clearAllCachedTitles()

    @Query("SELECT * FROM 'remote_key_toprated_tv' WHERE cachedTitleId = :id")
    abstract suspend fun getRemoteKeyById(id: Long): RemoteKeyTopRatedTV?

    @Query("DELETE FROM 'remote_key_toprated_tv'")
    abstract suspend fun clearRemoteKeys()

    @Query("SELECT createdOn FROM 'remote_key_toprated_tv' ORDER BY createdOn DESC LIMIT 1")
