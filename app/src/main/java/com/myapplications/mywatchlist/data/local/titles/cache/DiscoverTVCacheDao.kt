...
@Dao
abstract class DiscoverTVCacheDao :
    ...
    @Query("SELECT * FROM 'title_item_cache_discover_tv' ORDER BY page, id")
    abstract fun getCachedTitles(): PagingSource<Int, TitleItemCacheDiscoverTVFull>

    @Query("DELETE FROM 'title_item_cache_discover_tv'")
    abstract suspend fun clearAllCachedTitles()

    @Query("SELECT * FROM 'remote_key_discover_tv' WHERE cachedTitleId = :id")
    abstract suspend fun getRemoteKeyById(id: Long): RemoteKeyDiscoverTV?

    @Query("DELETE FROM 'remote_key_discover_tv'")
    abstract suspend fun clearRemoteKeys()

    @Query("SELECT createdOn FROM 'remote_key_discover_tv' ORDER BY createdOn DESC LIMIT 1")
