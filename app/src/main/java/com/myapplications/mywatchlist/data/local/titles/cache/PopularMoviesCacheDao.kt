...
@Dao
abstract class PopularMoviesCacheDao :
    BaseCacheDao<TitleItemCachePopularMovie, GenreForCacheItemPopularMovie, RemoteKeyPopularMovie> {
    ...
    @Query("SELECT * FROM 'title_item_cache_popular_movie' ORDER BY page, id")
    abstract fun getCachedTitles(): PagingSource<Int, TitleItemCachePopularMovieFull>

    @Query("DELETE FROM 'title_item_cache_popular_movie'")
    abstract suspend fun clearAllCachedTitles()

    @Query("SELECT * FROM 'remote_key_popular_movie' WHERE cachedTitleId = :id")
    abstract suspend fun getRemoteKeyById(id: Long): RemoteKeyPopularMovie?

    @Query("DELETE FROM 'remote_key_popular_movie'")
    abstract suspend fun clearRemoteKeys()

    @Query("SELECT createdOn FROM 'remote_key_popular_movie' ORDER BY createdOn DESC LIMIT 1")
